...
- [x] T052 [P] Run [quickstart.md](./quickstart.md#sc-002--under-150-ms) SC-002 on the baseline host — build the fixture first (a 10-tab workspace, history for 20 workspaces), discard the first invocation, report the **80th percentile** of 20 warm samples (the 16th once sorted), and confirm it is under 150 ms. Record the number in `SESSION.local.md`; re-measure on a quieter machine before treating a result near the threshold as a regression. Samples above the 80th percentile belong to herdr's CLI round-trip rather than to this plugin — [research.md](./research.md#sc-002-measurement) records the decomposition, and it is why the criterion is not the maximum
